...
# testimg_Label=mp.imread('TestSet/1m.jpg')
testimg_Label=mp.imread('TestSet/butterfly_GT.bmp')
# testimg_Label=mp.imread('TestSet/lenna.bmp')
# testimg_Label=mp.imread('TestSet/10.bmp')
# testimg_Label=mp.imread('TestSet/4.bmp')
testimg_Label_Ycbcr=cv2.cvtColor(testimg_Label,cv2.COLOR_RGB2YCrCb)
# testimg_Label=cv2.imread('TestSet/1.png')
...
# loss=tf.reduce_mean(tf.square(Yp-(OUT/127.5-1)))/3
# TrainStep=tf.train.AdamOptimizer(0.0001).minimize(loss)
# bicubicloss=tf.reduce_mean(tf.square(Yp-Resize))/3
...
iter=13
bicubic,testimg=sess.run([Resize,OUT], feed_dict={Xp: testimg_Data[np.newaxis,:,:,:], Yp: testimg_Label[np.newaxis,:,:,:]})
testimg=cv2.cvtColor(testimg[0,:,:,:],cv2.COLOR_RGB2YCrCb)
bicubic=cv2.cvtColor((bicubic[0,:,:,:]+1)*127.5,cv2.COLOR_RGB2YCrCb)

err=np.mean(np.square(testimg_Label_Ycbcr[:,:,0]-testimg[:,:,0]))
errbicubic=np.mean(np.square(testimg_Label_Ycbcr[:,:,0]-bicubic[:,:,0]))
