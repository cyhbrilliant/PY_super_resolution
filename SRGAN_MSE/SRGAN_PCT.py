...
def Generator(Xp):
    ...
    varlist.append(WDEConv1)
    varlist.append(BDEConv1)

    WDEConv2 = weight_variable([9, 9, 3, 256])
    BDEConv2 = bias_variable([3])
    varlist.append(WDEConv2)
    varlist.append(BDEConv2)
    # OUT = (tf.nn.tanh(tf.nn.conv2d_transpose(DEConv1, WDEConv2, output_shape=[batch_num, H * Scale, W * Scale, 3],strides=[1, 2, 2, 1], padding='SAME') + BDEConv2) + 1) * 127.5
...
loss_mse=tf.reduce_mean(tf.reduce_sum(tf.square(vgg2.conv2_2-vgg1.conv2_2)))
...
# saver.restore(sess,'SRGAN_MSE_session/SRGAN_MSE_MODEL_Std91.ckpt')
for iter in range(100000):
    print('\n',iter)
    Data_batch,Label_batch=getBatch(batch_num)
    error,result=sess.run([loss_mse,TrainStep_mse],feed_dict={Xp:Data_batch,Yp:Label_batch})
    print(error)
    # for i in range(Ktrain):
    #     Data_batch, Label_batch = getBatch(batch_num)
    #     errD,resultD=sess.run([loss_D,TrainStep_D],feed_dict={Xp:Data_batch,Yp:Label_batch})
    # Data_batch, Label_batch = getBatch(batch_num)
    # errLSR, resultLSR = sess.run([loss_LSR, TrainStep_LSR], feed_dict={Xp: Data_batch, Yp: Label_batch})
    ...
    if (iter+1)%5000==0:
        ...
        #无是mse预训练
        #3开启2-2pct预训练
        path = saver.save(sess,'SRGAN_MSE_session/SRGAN_MSE_MODEL_Std91_3.ckpt')
        print(path)