...
TrainStep_mse=tf.train.RMSPropOptimizer(0.0001).minimize(loss_mse)

loss_content=tf.reduce_mean((tf.square(vgg2.conv2_2-vgg1.conv2_2)))
TrainStep_content=tf.train.RMSPropOptimizer(0.0001).minimize(loss_content)

loss_G=tf.reduce_mean(tf.log(tf.clip_by_value(1-DOUT_G,1e-10,1.0))+100*tf.reduce_mean(tf.square(Yp-GOUT)))
loss_LSR=0.001*loss_G+loss_content
TrainStep_LSR=tf.train.RMSPropOptimizer(0.0001).minimize(loss_LSR,var_list=Gvarlist)
TrainStep_G=tf.train.RMSPropOptimizer(0.0001).minimize(loss_G,var_list=Gvarlist)
...
TrainStep_D=tf.train.RMSPropOptimizer(0.0001).minimize(loss_D,var_list=Dvarlist)
#
#
#
...
for iter in range(200000):
    ...
    for i in range(Ktrain):
        # Data_batch, Label_batch = getBatch(batch_num)
        errD,resultD=sess.run([loss_D,TrainStep_D],feed_dict={Xp:Data_batch,Yp:Label_batch/127.5-1.,train_mode:False})
    errLSR, resultLSR = sess.run([loss_G, TrainStep_G], feed_dict={Xp: Data_batch, Yp: Label_batch/127.5-1.,train_mode:False})
    ...
    if (iter+1)%1000==0:
        path = saver.save(sess,'UpscaleSRGAN_session/SRGAN_VGG22_imagenet_Full5.ckpt')
        print(path)