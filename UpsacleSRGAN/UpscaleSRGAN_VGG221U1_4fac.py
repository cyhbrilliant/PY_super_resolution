...
def Generator(Xp):
    ...
    DEConv2=tf.image.resize_nearest_neighbor(Res3,size=[H * Scale, W * Scale])

    # Wconv2 = weight_variable([3, 3, 64, 64])
    # Bconv2 = bias_variable([64])
    # conv2 = Leakyrelu(tf.nn.conv2d(DEConv2, Wconv2, strides=[1, 1, 1, 1], padding='SAME') + Bconv2)
...
loss_LSR=loss_G+loss_content
loss_LSRsm=0.001*loss_G+loss_content
...
# saver.restore(sess,'UpscaleSRGAN_session/SRGAN_VGG22_imagenet_Full7.ckpt')
...
for iter in range(200000):
    ...
    # errLSR, resultLSR = sess.run([loss_G, TrainStep_LSR], feed_dict={Xp: Data_batch/127.5-1., Yp: Label_batch/127.5-1.,train_mode:False})
    ...
    if (iter+1)%1000==0:
        path = saver.save(sess,'UpscaleSRGAN_session/SRGAN_VGG22_imagenet_Full7x.ckpt')
        print(path)