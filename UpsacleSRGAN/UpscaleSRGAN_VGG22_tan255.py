...
loss_G=-tf.reduce_sum(tf.log(tf.clip_by_value(DOUT_G,1e-10,1.0)))
...
loss_D=-tf.reduce_sum(tf.log(tf.clip_by_value(DOUT_D,1e-10,1.0))+tf.log(tf.clip_by_value(1-DOUT_G,1e-10,1.0)))
...
for iter in range(200000):
    ...
    if (iter+1)%1000==0:
        path = saver.save(sess,'UpscaleSRGAN_session/SRGAN_VGG22_imagenet_Full4.ckpt')
        print(path)