...
# testimg_Label=mp.imread('13.bmp')
...
testimg_Label=cv2.resize(testimg_Label,dsize=(int(testimg_Label.shape[1]/6)*6,int(testimg_Label.shape[0]/6)*6))/127.5-1.
# testimg_Label=cv2.resize(testimg_Label,dsize=(1016,912))
testimg_Data=cv2.resize(testimg_Label,dsize=(int(testimg_Label.shape[1]/6),int(testimg_Label.shape[0]/6)),interpolation=cv2.INTER_AREA)
...
# saver.restore(sess,'UpscaleSRGAN_session/SRGAN_VGG22_imagenet_Full7x.ckpt')#PCT
saver.restore(sess,'M500/M4000.ckpt')
...
cv2.imwrite('OUTPUT/bicubic.jpg',(bicubic+1)*127.5)
