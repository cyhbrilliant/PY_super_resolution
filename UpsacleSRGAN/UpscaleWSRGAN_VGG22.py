...
loss_G=tf.reduce_sum(-DOUT_G+100*tf.reduce_mean((tf.square(vgg2.conv2_2-vgg1.conv2_2))))
...
loss_D=-tf.reduce_sum(DOUT_D-DOUT_G)
...
for iter in range(200000):
    ...
    # print(error)
    # Data_batch,Label_batch=getBatch(batch_num)
    # error,result=sess.run([loss_content,TrainStep_content],feed_dict={Xp:Data_batch,Yp:Label_batch/127.5-1.,train_mode:False})
    # print(error)
    Data_batch, Label_batch = getBatch(batch_num)
    ...
    if (iter+1)%1000==0:
        path = saver.save(sess,'UpscaleSRGAN_session/WSRGAN_VGG22_imagenet_Full4.ckpt')
        print(path)