...
def Discriminator(GXp,Yp):
    ...
    Wconv4=weight_variable([3,3,128,256])
    Bconv4=bias_variable([256])
    ...
    varlist = [Wconv1, Bconv1, Wconv2, Bconv2, Wconv3, Bconv3, Wconv4, Bconv4]

    for reslayer in range(5):
        CH1WRes1 = weight_variable([1, 1, 256, 32])
        CH1BRes1 = bias_variable([32])
        GCH1Res1 = Leakyrelu(
            batch_norm(tf.nn.conv2d(GConv4, CH1WRes1, strides=[1, 1, 1, 1], padding='SAME') + CH1BRes1, BNtrain))
        DCH1Res1 = Leakyrelu(
            batch_norm(tf.nn.conv2d(DConv4, CH1WRes1, strides=[1, 1, 1, 1], padding='SAME') + CH1BRes1, BNtrain))

        CH2WRes1 = weight_variable([1, 1, 256, 32])
        CH2BRes1 = bias_variable([32])
        GCH2Res1 = Leakyrelu(
            batch_norm(tf.nn.conv2d(GConv4, CH2WRes1, strides=[1, 1, 1, 1], padding='SAME') + CH2BRes1, BNtrain))
        DCH2Res1 = Leakyrelu(
            batch_norm(tf.nn.conv2d(DConv4, CH2WRes1, strides=[1, 1, 1, 1], padding='SAME') + CH2BRes1, BNtrain))

        CH3WRes1 = weight_variable([1, 1, 256, 32])
        CH3BRes1 = bias_variable([32])
        GCH3Res1 = Leakyrelu(
            batch_norm(tf.nn.conv2d(GConv4, CH3WRes1, strides=[1, 1, 1, 1], padding='SAME') + CH3BRes1, BNtrain))
        DCH3Res1 = Leakyrelu(
            batch_norm(tf.nn.conv2d(DConv4, CH3WRes1, strides=[1, 1, 1, 1], padding='SAME') + CH3BRes1, BNtrain))

        CH2WRes2 = weight_variable([3, 3, 32, 32])
        CH2BRes2 = bias_variable([32])
        GCH2Res2 = Leakyrelu(
            batch_norm(tf.nn.conv2d(GCH2Res1, CH2WRes2, strides=[1, 1, 1, 1], padding='SAME') + CH2BRes2, BNtrain))
        DCH2Res2 = Leakyrelu(
            batch_norm(tf.nn.conv2d(DCH2Res1, CH2WRes2, strides=[1, 1, 1, 1], padding='SAME') + CH2BRes2, BNtrain))

        CH3WRes2 = weight_variable([3, 3, 32, 48])
        CH3BRes2 = bias_variable([48])
        GCH3Res2 = Leakyrelu(
            batch_norm(tf.nn.conv2d(GCH3Res1, CH3WRes2, strides=[1, 1, 1, 1], padding='SAME') + CH3BRes2, BNtrain))
        DCH3Res2 = Leakyrelu(
            batch_norm(tf.nn.conv2d(DCH3Res1, CH3WRes2, strides=[1, 1, 1, 1], padding='SAME') + CH3BRes2, BNtrain))

        CH3WRes3 = weight_variable([3, 3, 48, 64])
        CH3BRes3 = bias_variable([64])
        GCH3Res3 = Leakyrelu(
            batch_norm(tf.nn.conv2d(GCH3Res2, CH3WRes3, strides=[1, 1, 1, 1], padding='SAME') + CH3BRes3, BNtrain))
        DCH3Res3 = Leakyrelu(
            batch_norm(tf.nn.conv2d(DCH3Res2, CH3WRes3, strides=[1, 1, 1, 1], padding='SAME') + CH3BRes3, BNtrain))

        GCHConcat = tf.concat([GCH1Res1, GCH2Res2, GCH3Res3], axis=3)
        DCHConcat = tf.concat([DCH1Res1, DCH2Res2, DCH3Res3], axis=3)

        # conv
        WResnet = weight_variable([1, 1, 128, 256])
        BResnet = bias_variable([256])
        GResnet_inner = batch_norm(tf.nn.conv2d(GCHConcat, WResnet, strides=[1, 1, 1, 1], padding='SAME') + BResnet,
                                  BNtrain)
        DResnet_inner = batch_norm(tf.nn.conv2d(DCHConcat, WResnet, strides=[1, 1, 1, 1], padding='SAME') + BResnet,
                                  BNtrain)
        GConv4 = GResnet_inner + GConv4
        DConv4= DResnet_inner + DConv4
    ...
    GConv6=Leakyrelu(batch_norm(tf.nn.conv2d(GConv4,Wconv6,strides=[1,2,2,1],padding='SAME')+Bconv6,BNtrain))
    DConv6=Leakyrelu(batch_norm(tf.nn.conv2d(DConv4,Wconv6,strides=[1,2,2,1],padding='SAME')+Bconv6,BNtrain))
    varlist.append(Wconv6)
    varlist.append(Bconv6)
    ...
    for reslayer in range(5):
        ...
        CH1BRes1 = bias_variable([32])
        GCH1Res1 = Leakyrelu(
            batch_norm(tf.nn.conv2d(GConv6, CH1WRes1, strides=[1, 1, 1, 1], padding='SAME') + CH1BRes1, BNtrain))
        DCH1Res1 = Leakyrelu(
            batch_norm(tf.nn.conv2d(DConv6, CH1WRes1, strides=[1, 1, 1, 1], padding='SAME') + CH1BRes1, BNtrain))

        CH2WRes1 = weight_variable([1, 1, 256, 32])
        CH2BRes1 = bias_variable([32])
        GCH2Res1 = Leakyrelu(
            batch_norm(tf.nn.conv2d(GConv6, CH2WRes1, strides=[1, 1, 1, 1], padding='SAME') + CH2BRes1, BNtrain))
        DCH2Res1 = Leakyrelu(
            batch_norm(tf.nn.conv2d(DConv6, CH2WRes1, strides=[1, 1, 1, 1], padding='SAME') + CH2BRes1, BNtrain))

        CH3WRes1 = weight_variable([1, 1, 256, 32])
        CH3BRes1 = bias_variable([32])
        GCH3Res1 = Leakyrelu(
            batch_norm(tf.nn.conv2d(GConv6, CH3WRes1, strides=[1, 1, 1, 1], padding='SAME') + CH3BRes1, BNtrain))
        DCH3Res1 = Leakyrelu(
            batch_norm(tf.nn.conv2d(DConv6, CH3WRes1, strides=[1, 1, 1, 1], padding='SAME') + CH3BRes1, BNtrain))
        ...
        DResnet_inner = batch_norm(tf.nn.conv2d(DCHConcat, WResnet, strides=[1, 1, 1, 1], padding='SAME') + BResnet,
                                  BNtrain)
        GConv6 = GResnet_inner + GConv6
        DConv6= DResnet_inner + DConv6
    ...
    GConv8=Leakyrelu(batch_norm(tf.nn.conv2d(GConv6,Wconv8,strides=[1,2,2,1],padding='SAME')+Bconv8,BNtrain))
    DConv8=Leakyrelu(batch_norm(tf.nn.conv2d(DConv6,Wconv8,strides=[1,2,2,1],padding='SAME')+Bconv8,BNtrain))
    GConv8_line=tf.reshape(GConv8,shape=[-1,18432])
    DConv8_line=tf.reshape(DConv8,shape=[-1,18432])

    varlist.append(Wconv8)
    varlist.append(Bconv8)
...
loss_LSRsm=loss_G+0.001*loss_content
...
saver.restore(sess,'UpscaleSRGAN_session/DC.ckpt')
...
for iter in range(100000):
    ...
    #summary_str = sess.run(merged_summary_op,feed_dict={Xp:data_batch,Yp:label_batch})
    ...
    if (iter+1)%5000==0:
        # path = saver.save(sess,'UpscaleSRGAN_session/SRGAN_VGG54_imagenet_BGDx10_area.ckpt')
        # path = saver.save(sess,'UpscaleSRGAN_session/SRGAN_VGG22_imagenet_BGDx10_areaM.ckpt')#50
        # path = saver.save(sess,'UpscaleSRGAN_session/SRGAN_VGG22_imagenet_BGDx10_areaMB.ckpt')#50
        # path = saver.save(sess,'UpscaleSRGAN_session/SRGAN_VGG22_imagenet_BGDx10_areaMB2.ckpt')#50
        # path = saver.save(sess,'UpscaleSRGAN_session/SRGAN_VGG22_imagenet_BGDx30_areaM.ckpt')
        # path = saver.save(sess,'UpscaleSRGAN_session/SRGAN_MSE_imagenet_area.ckpt')
        # path = saver.save(sess,'UpscaleSRGAN_session/M'+str(iter+1+1000)+'.ckpt')
        path = saver.save(sess,'UpscaleSRGAN_session/DC.ckpt')
